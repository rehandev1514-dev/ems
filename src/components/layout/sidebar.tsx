...
const nav: Array<{ to: string; label: string; icon: typeof LayoutDashboard; roles: Role[] }> = [
  ...
  { to: "/attendance", label: "Attendance", icon: CalendarCheck, roles: ["admin", "employee", "hr", "manager"] },
  { to: "/leaves", label: "Leaves", icon: Plane, roles: ["admin", "employee", "hr", "manager"] },
  { to: "/projects", label: "Projects", icon: FolderKanban, roles: ["admin", "employee", "manager"] },
  { to: "/tasks", label: "Tasks", icon: ListChecks, roles: ["admin", "employee", "manager"] },
  { to: "/assets", label: "Assets", icon: Boxes, roles: ["admin", "hr"] },
  { to: "/documents", label: "Documents", icon: FileText, roles: ["admin", "employee", "hr"] },
  { to: "/notifications", label: "Notifications", icon: Bell, roles: ["admin", "employee", "hr", "manager"] },
    ...
    <aside className="hidden md:flex w-64 min-w-[16rem] shrink-0 flex-col border-r border-border bg-sidebar/80 backdrop-blur-xl">
      <div className="flex items-center gap-2.5 px-5 h-16 border-b border-sidebar-border">
        ...
        <div className="leading-tight">
          <div className="text-sm font-semibold tracking-tight">VertexEMS</div>
          <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Code Vertex</div>
        </div>
      </div>
