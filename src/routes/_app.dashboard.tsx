...
import {
  Users, Building2, CalendarCheck, Plane, FolderKanban, ListChecks, Boxes,
  TrendingUp, Plus, UserPlus, FileBarChart, Clock, CheckCircle2,
} from "lucide-react";
import {
  employees, departments, attendance, leaveRequests, projects, tasks, assets,
  employeeById, projectById,
} from "@/lib/mock-data";
...
function AdminDashboard() {
  ...
  const attendanceTrend = useMemo(() => {
    ...
    return Object.values(map).sort((a, b) => a.date.localeCompare(b.date)).slice(-14).map((r) => ({
      ...r, label: new Date(r.date).toLocaleDateString("en-GB", { day: "2-digit", month: "short" }),
    }));
  }, []);
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Button variant="outline" asChild><Link to="/reports"><FileBarChart className="size-4" /> Reports</Link></Button>
            <Button className="gradient-primary text-primary-foreground shadow-glow" asChild>
              <Link to="/employees"><UserPlus className="size-4" /> Add employee</Link>
            </Button>
          </>
        }
      />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total employees" value={employees.length} icon={Users} accent="primary" trend={{ value: "+2 this month", positive: true }} />
        <StatCard label="Active" value={activeEmps} icon={CheckCircle2} accent="success" trend={{ value: `${Math.round((activeEmps / employees.length) * 100)}% of total`, positive: true }} />
        <StatCard label="Departments" value={departments.length} icon={Building2} accent="secondary" />
        <StatCard label="Attendance rate" value={`${attendanceRate}%`} icon={CalendarCheck} accent="info" trend={{ value: "Today", positive: true }} />
        <StatCard label="Leave requests" value={pendingLeaves} icon={Plane} accent="warning" trend={{ value: "Pending review" }} />
        <StatCard label="Active projects" value={activeProjects} icon={FolderKanban} accent="primary" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-6">
        <Card className="lg:col-span-2 p-5 glass shadow-elevated">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2 text-xs">
              <span className="flex items-center gap-1.5"><span className="size-2 rounded-full bg-[--color-chart-1]" /> Present</span>
              <span className="flex items-center gap-1.5"><span className="size-2 rounded-full bg-[--color-chart-2]" /> Late</span>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={attendanceTrend}>
              ...
              <Area type="monotone" dataKey="late" stroke="var(--color-chart-2)" fill="url(#g2)" strokeWidth={2} />
        ...
        <Card className="p-5 glass shadow-elevated">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <PieChart>
              <Pie data={deptDist} dataKey="value" nameKey="name" innerRadius={55} outerRadius={85} paddingAngle={2}>
                {deptDist.map((_, i) => <Cell key={i} fill={colors[i % colors.length]} stroke="var(--color-background)" />)}
              </Pie>
              <Tooltip contentStyle={{ background: "var(--color-popover)", border: "1px solid var(--color-border)", borderRadius: 8, fontSize: 12 }} />
            </PieChart>
          </ResponsiveContainer>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4">
        ...
        <Card className="lg:col-span-2 p-5 glass shadow-elevated">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button variant="ghost" size="sm" asChild><Link to="/projects">View all <TrendingUp className="size-4" /></Link></Button>
          </div>
              ...
              <div key={p.id}>
                <div className="flex items-center justify-between mb-1.5">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">{p.client} · Due {formatDate(p.deadline)}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <StatusBadge status={p.status} />
                    <span className="text-sm font-semibold tabular-nums w-10 text-right">{p.progress}%</span>
                  </div>
                </div>
            ...
            {leaveRequests.slice(0, 5).map((l) => {
              ...
              return (
                <div key={l.id} className="flex items-center gap-3">
                  <Avatar className="size-9"><AvatarFallback className="text-xs">{initials(emp?.fullName ?? "?")}</AvatarFallback></Avatar>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium truncate">{emp?.fullName}</div>
                    <div className="text-xs text-muted-foreground capitalize">{l.type} leave · {l.days}d · {formatDate(l.startDate)}</div>
                  </div>
        ...
        <Card className="p-5 glass shadow-elevated">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <Link key={a.to} to={a.to} className="group relative rounded-xl border border-border bg-card hover:border-primary/40 hover:bg-accent/40 transition-all p-4">
...
function EmployeeDashboard() {
  ...
  const upcoming = myTasks.filter((t) => t.status !== "done").sort((a, b) => a.deadline.localeCompare(b.deadline)).slice(0, 5);
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Button variant="outline" asChild><Link to="/leaves"><Plane className="size-4" /> Apply leave</Link></Button>
            <Button className="gradient-primary text-primary-foreground shadow-glow" asChild>
              <Link to="/attendance"><Clock className="size-4" /> Mark attendance</Link>
            </Button>
          </>
        }
      />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="My attendance" value={`${presentDays}/${myAttendance.length}`} icon={CalendarCheck} accent="success" trend={{ value: "Last 30 days" }} />
        <StatCard label="Assigned tasks" value={myTasks.length} icon={ListChecks} accent="primary" trend={{ value: `${myTasks.filter((t) => t.status === "in_progress").length} in progress` }} />
        <StatCard label="Pending requests" value={myLeaves.filter((l) => l.status === "pending").length} icon={Plane} accent="warning" />
        <StatCard label="Active projects" value={myProjects.length} icon={FolderKanban} accent="info" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-6">
        <Card className="lg:col-span-2 p-5 glass shadow-elevated">
          ...
          <div className="space-y-3">
            {upcoming.length === 0 && <p className="text-sm text-muted-foreground">All caught up — nothing pending. 🎉</p>}
            {upcoming.map((t) => (
              <div key={t.id} className="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-accent/40 transition-colors">
                <div className="size-10 rounded-lg gradient-primary/10 bg-primary/10 flex items-center justify-center">
                  <ListChecks className="size-5 text-primary" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium truncate">{t.title}</div>
                  <div className="text-xs text-muted-foreground">{projectById(t.projectId)?.name} · Due {formatDate(t.deadline)}</div>
                </div>
