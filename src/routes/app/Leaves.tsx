      ...
      <PageHeader
        ...
        description={
          isAdmin
            ...
            : "Apply for leave and track your balance and history."
        }
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: "Casual balance", value: "5 days", accent: "from-secondary/20" },
          {
            ...
            value: mine.filter((l) => l.status === "pending").length,
            accent: "from-[--color-warning]/20",
          },
        ].map((s) => (
          <Card key={s.label} className={`relative overflow-hidden p-5 glass shadow-elevated`}>
            <div
              className={`absolute -top-10 -right-10 size-32 rounded-full blur-3xl bg-linear-to-br ${s.accent} to-transparent opacity-60`}
            />
...
function ApplyLeaveDialog() {
  ...
  const applyMutation = useMutation({
    mutationFn: (payload: { type: string; startDate: string; endDate: string; reason: string }) =>
